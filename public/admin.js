...
const mainShell = document.querySelector(".admin-shell");
const logoutBtn = document.getElementById("admin-logout");

const cacheKey = "x-profile-sorter:last-result:v21";
const authKey = "x-sort:admin-password";
...
function renderAnalyzeResult(data, shouldScroll = false) {
  ...
  if (termTitle) termTitle.textContent = `x-sort — admin console — @${data.username} — ${data.total} posts`;
  ...
  // Only show Sync Supabase button for ice_bearcute (case-insensitive)
  if (syncButton) {
...
if (syncButton) {
  syncButton.addEventListener("click", async () => {
    ...
    const password = localStorage.getItem(authKey);
    ...
    if (!password) {
      alert("Admin password missing. Please reload and log in again.");
      return;
    }
